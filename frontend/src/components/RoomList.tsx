...
export default function RoomList({
  ...
  const toggleMemberList = (roomId: string) => {
    setMemberStates((prev) => {
      ...
      if (!state || !state.showList) {
        // Fetch members if not already loaded or list is hidden
        fetchMembers(roomId)
        return {
          ...prev,
  ...
  const handleDoubleClick = (roomId: string, e: React.MouseEvent) => {
    e.stopPropagation()
      ...
      <div className="p-4 border-b border-gray-200/60">
        {!showCreate ? (
          <button
            ...
            className="w-full py-3 bg-gradient-to-r from-blue-500 to-indigo-500 text-white rounded-xl hover:from-blue-600 hover:to-indigo-600 transition-all duration-200 font-medium shadow-lg shadow-blue-500/25 hover:shadow-blue-500/40 flex items-center justify-center gap-2"
          >
          ...
          <form onSubmit={handleCreate} className="space-y-3">
            ...
            <div className="flex gap-2">
              <button
                type="submit"
                className="flex-1 py-3 bg-gradient-to-r from-blue-500 to-indigo-500 text-white rounded-xl hover:from-blue-600 hover:to-indigo-600 font-medium"
              >
                创建
              </button>
              <button
              ...
              <div key={room.id} className="space-y-1">
                ...
                >
                  <div
                    ...
                    onDoubleClick={(e) => handleDoubleClick(room.id, e)}
                  >
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      {/* Room Icon */}
                      <div className={`w-11 h-11 rounded-xl bg-gradient-to-br ${isSelected ? 'from-white/20 to-white/10' : colorClass} flex items-center justify-center text-white font-bold text-sm shadow-md flex-shrink-0`}>
                        {room.name.slice(0, 2).toUpperCase()}
                      </div>

                      <div className="flex-1 min-w-0">
                        <div className={`font-semibold truncate ${isSelected ? 'text-white' : 'text-gray-800'}`}>
                          {room.name}
                        </div>
                        <div className={`text-sm flex items-center gap-1 ${isSelected ? 'text-white/70' : 'text-gray-500'}`}>
                ...
                {/* Bot Panel */}
                {showList && (
                  <div className="ml-3 p-3 bg-gray-50 rounded-2xl border border-gray-100">
                    {showForm && isFormForRoom(room.id) ? (
                      <div className="space-y-3">
                        <h4 className="text-sm font-semibold text-gray-700">创建AI机器人</h4>
                        <input
                          type="text"
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className="flex-1 py-2 bg-gradient-to-r from-blue-500 to-indigo-500 text-white text-sm rounded-xl hover:from-blue-600 hover:to-indigo-600 disabled:from-gray-300 disabled:to-gray-400 disabled:cursor-not-allowed font-medium"
                          >
                            {state?.loading ? '创建中...' : '创建'}
                          </button>
                          <button
                            onClick={() => hideBotForm(room.id)}
                            className="flex-1 py-2 bg-gray-200 text-gray-700 text-sm rounded-xl hover:bg-gray-300 font-medium"
                          >
                            取消
                          </button>
                        </div>
                      ...
                      <>
                        <div className="space-y-2 mb-3">
                          {bots.map((bot) => (
                            <div
                              key={bot.id}
                              className="flex items-center justify-between bg-white px-3 py-2 rounded-xl border border-gray-200"
                            >
                        ...
                        <button
                          ...
                          className="w-full py-2.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white text-sm rounded-xl hover:from-emerald-600 hover:to-teal-600 font-medium shadow-md shadow-emerald-500/20"
                        >
                          + 添加机器人
                        </button>
                      </>
                ...
                {(() => {
                  ...
                  return (
                    <div className="ml-3 p-3 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl border border-indigo-100">
                      ...
                      {membersLoading ? (
                        ...
                        <div className="space-y-1.5">
                          ...
                          {bots.length > 0 && (
                            <>
                              <div className="text-xs font-medium text-gray-500 mb-1.5 px-1">机器人</div>
                              {bots.map((bot) => (
                                <div
                                  key={bot.id}
                                  className="flex items-center gap-2 bg-gradient-to-r from-emerald-50 to-teal-50 px-3 py-2 rounded-xl border border-emerald-200"
                                >
                                  <span className="text-lg">{bot.avatar}</span>
                                  <span className="text-sm font-medium text-gray-700">{bot.name}</span>
                                  <span className="text-xs bg-emerald-100 text-emerald-600 px-2 py-0.5 rounded-full">AI</span>
                                </div>
                              ))}
                            </>
                          )}
                          ...
                          {members.length > 0 && (
                            <>
                              {bots.length > 0 && <div className="text-xs font-medium text-gray-500 mb-1.5 mt-2 px-1">用户</div>}
                              {members.map((member) => (
                                <div
                                  key={member}
                                  className="flex items-center gap-2 bg-white px-3 py-2 rounded-xl border border-indigo-100"
                                >
